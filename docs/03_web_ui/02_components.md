...
- ログリンク
- 設定リンク（階層構造対応）
  - HTTP設定（Virtual Hosts階層構造対応）
  - DNS設定（Domains階層構造対応）
  - その他サーバー設定

**階層構造の特徴:**
- Virtual Hosts: 各仮想ホストごとの詳細設定を階層的に表示
- Domains: 各DNSドメインごとの設定を階層的に表示（PR #76でDNS設定配下に統合）
- 展開/折りたたみ可能なサブメニュー

**注記:** PR #76により、従来の独立した「DNSレコードリンク」は「DNS設定」→「Domains」の階層構造に統合されました。
